...
def evaluate_kfold_mean(kf_metrics, dataset_idx=0):
    ...
    rb = xlrd.open_workbook('mean_r_result.xls')
    wb = copy(rb)
...
def average_precision(output, target, difficult_examples=True):
    # sort examples
    import torch
    sorted, indices = torch.sort(torch.from_numpy(output), dim=0, descending=True)
...
#asl
def average_precision1(output, target):
    epsilon = 1e-8
...
def mAP_asl(targs, preds):
    ...
    for k in range(preds.shape[1]):
        ...
        targets = targs[:, k]
        # compute average precision
        ap[k] = average_precision1(scores, targets)
    return 100 * ap.mean()

def evaluate(y_test, output, predict, model='mlgcn'):
    metrics = dict()

    if model == 'mlgcn':
        metrics['mAP'] = mAP_mlgcn(y_test, output)
    else:
        metrics['mAP'] = mAP_asl(y_test, output)
    return metrics

def evaluate1(y_test, output, predict):
    """
    评估模型
    :param y_test:{0,1}
    :param output:（-1，1）
    ...
    #metrics['average_precision1'] = label_ranking_average_precision_score(y_test, output)
