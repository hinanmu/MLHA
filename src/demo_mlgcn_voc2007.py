...
parser.add_argument('--eval', default=4, type=int, metavar='N',
...
def evaluate_defense_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        metrics['mean_r'] = np.mean(mean_r_t)
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=1, args=args, sheet=0, filename='defense_result.xls')
...
def display_adv(state):
    ...
    with torch.no_grad():
        ...
        for batch_adv_x, batch_test_x in zip(dl1, dl2):
            ...
            plt.savefig('../attack_result/mlgcn_voc2007_{}_adv_conf_{}.pdf'.format(args.adv_method, display_idx), format='pdf', bbox_inches='tight')
            plt.clf()
            ...
            plt.xlim(0, 1)
            plt.xticks([0.0, 0.5, 1.0])
            plt.savefig('../attack_result/mlgcn_voc2007_test_conf_{}.pdf'.format(args.adv_method, display_idx), format='pdf', bbox_inches='tight')
            plt.clf()
