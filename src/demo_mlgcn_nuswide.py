...
def evaluate_model(model):
    ...
    test_dataset = NusWide(args.data, phase='Test', inp_name='../data/NUSWIDE/glove_word2vec.pkl')
...
def evaluate_defense_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        metrics['mean_r'] = np.mean(mean_r_t)
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=11, args=args, sheet=0, filename='defense_result.xls')
