...
parser.add_argument('--adv_file_path', default='../data/VOC2012/classification_asl_adv.csv', type=str, metavar='N',
                    help='all image names and their labels ready to attack')
parser.add_argument('--adv_save_x', default='../adv_save/asl/VOC2012/', type=str, metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    # value is {-1,1}
    np.save('../adv_save/asl/VOC2012/y_target.npy', y_target)
    np.save('../adv_save/asl/VOC2012/y.npy', y)
...
def evaluate_adv(state):
    ...
    adv_dataset = Voc2012Classification(args.data, 'asl_adv', inp_name='../data/VOC2012/voc_glove_word2vec.pkl')
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/asl_voc2012_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
        ...
        write_to_excel(metrics, base_row=26, args=args, sheet=i, filename='max_r_result.xls')
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/asl_voc2012_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
        ...
        print()
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=26, args=args, sheet=i, filename='mean_r_result.xls')
...
def evaluate_defense_adv(state):
    ...
    adv_dataset = Voc2012Classification(args.data, 'asl_adv')
    adv_dataset.transform = data_transforms
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        metrics['mean_r'] = np.mean(mean_r_t)
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=26, args=args, sheet=0, filename='defense_result.xls')
...
def main():
    ...
    # value is {-1,1}
    y_target = np.load('../adv_save/asl/VOC2012/y_target.npy')
    y = np.load('../adv_save/asl/VOC2012/y.npy')
