...
parser.add_argument('--adv_method', default='FGSM', type=str, metavar='N',
                    help='attack method: ml_cw, ml_deepfool, ml_lp')
...
def evaluate_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        write_to_excel(metrics, base_row=16, args=args, sheet=i, filename='max_r_result.xls')
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        print()
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=16, args=args, sheet=i, filename='mean_r_result.xls')
...
def evaluate_defense_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        metrics['mean_r'] = np.mean(mean_r_t)
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=16, args=args, sheet=0, filename='defense_result.xls')
