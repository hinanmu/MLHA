...
parser.add_argument('--adv_batch_size', default=32, type=int,
                    metavar='N', help='batch size 32')
parser.add_argument('--workers', default=8, type=int, metavar='N',
                    help='number of data loading workers (default: 4)')
parser.add_argument('--adv_method', default='FGSM', type=str, metavar='N',
                    help='attack method: ml_cw, ml_deepfool, mla_lp, FGSM, MI-FGSM')
parser.add_argument('--target_type', default='hide_all', type=str, metavar='N',
                    help='target method: hide_all')
parser.add_argument('--adv_file_path', default='../data/VOC2012/classification_mlgcn_adv.csv', type=str, metavar='N',
...
def evaluate_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/mlgcn_voc2012_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/mlgcn_voc2012_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
...
def evaluate_defense_adv(state):
    ...
    with torch.no_grad():
        for batch_adv_x, batch_test_x in zip(dl1, dl2):
            if use_gpu:
                batch_adv_x = batch_adv_x.cuda()
            batch_adv_x_defense = jpeg_denfense(batch_adv_x)

            adv_output.extend(model(batch_adv_x_defense).cpu().numpy())
            batch_adv_x = batch_adv_x.cpu().numpy()
            batch_test_x = batch_test_x[0][0].cpu().numpy()
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        metrics['mean_r'] = np.mean(mean_r_t)
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=6, args=args, sheet=0, filename='defense_result.xls')
